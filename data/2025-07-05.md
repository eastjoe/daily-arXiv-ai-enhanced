...
Motivation: 生物医学数据的快速增长导致研究领域碎片化，AI智能体通常依赖静态工具集，难以适应和扩展。

Method: 多智能体架构，包含进化模板库和动态工具库，能够自主改进自身能力。

Result: 在生物医学基准测试中取得了最先进的准确率，例如Humanity's Last Exam: Biomedicine (26%)，LAB-Bench: DBQA (54%)，LAB-Bench: LitQA (63%)，并随着经验的积累而持续改进性能。

Conclusion: STELLA，一个自我进化的AI智能体，在生物医学基准测试中取得了最先进的准确率，并随着经验的积累而持续改进性能。
...
<details>
  <summary>Details</summary>
Motivation: 为了消除冗余特征并保留相关特征，提高分类器的性能。

Method: 提出一种轻量级的基于规则的特征选择方法HCVR，结合P2P和P2T相关性，通过逆向消除冗余特征，并利用投票规则决定保留或丢弃特征。

Result: HCVR方法在SPAMBASE数据集上优于CFS、mRMR、MI、RFE、SFS和遗传算法等传统方法。

Conclusion: HCVR方法在SPAMBASE数据集上表现出优于传统非迭代式和迭代式特征选择方法的性能。
...
<details>
  <summary>Details</summary>
Motivation: Current LLMs are inefficient at reasoning, using fixed compute regardless of task complexity. This survey aims to improve the computational efficiency of LLM reasoning.

Method: The survey uses a two-tiered taxonomy to categorize efficient test-time compute methods and benchmarks leading proprietary LLMs across diverse datasets.

Result: The survey identifies critical trade-offs between reasoning performance and token usage, and discusses emerging trends such as hybrid thinking models and challenges for future work.

Conclusion: This survey reviews efficient test-time compute (TTC) strategies for improving the computational efficiency of Large Language Model (LLM) reasoning, categorizing methods into L1-controllability (fixed compute budgets) and L2-adaptiveness (dynamically scaled inference).  The review emphasizes practical control, adaptability, and scalability, benchmarking leading LLMs and highlighting trade-offs between reasoning performance and token usage.
...
<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在科学实验设计和结果解释方面的能力，而现有的评估方法成本高昂且难以实现。

Method: 提出SciGym基准，利用基于系统生物学标记语言的干燥实验室模拟生物系统，评估LLM在开放式科学发现任务中的迭代实验设计和分析能力。对六个前沿LLM在137个小型系统上进行了评估。

Result: 评估结果显示，虽然能力更强的模型表现更好，但所有模型的性能都随着系统复杂性的增加而显著下降。

Conclusion: 大型语言模型(LLM)在科学实验设计和结果分析方面仍有很大的改进空间，尤其是在复杂系统中。
...
<details>
  <summary>Details</summary>
Motivation: 现代AI模型缺乏动物般快速适应环境变化的能力，尤其在社会互动环境中。

Method: 文献综述和比较分析

Result: 提出了神经科学见解指导AI持续学习的议程，并指明了神经科学和AI的互利研究方向。

Conclusion: 该论文探讨了将神经科学的见解应用于人工智能持续学习领域以改进AI系统适应能力的可能性，并提出了神经科学和人工智能之间相互借鉴的研究议程。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在评估AI系统公平性时存在选择偏差和标签偏差，本文旨在利用审计研究数据改进AI系统的训练和评估，以减少算法歧视。

Method: 利用审计研究数据训练和评估自动化招聘算法，并引入基于个体处理效应估计方法的干预措施。

Result: 发现平衡基准比率的方法在传统度量下看似有效，但在适当的度量下仍存在约10%的差异；基于个体处理效应估计方法的干预措施能进一步减少算法歧视。

Conclusion: 本文研究了如何利用审计研究数据改进自动化招聘算法的训练和评估，发现常用的公平性干预方法（平衡基准比率）在传统度量下看似实现了平等，但在适当的度量下仍存在约10%的差异，并引入了基于个体处理效应估计方法的干预措施以进一步减少算法歧视。
...
<details>
  <summary>Details</summary>
Motivation: 改进大型语言模型的数学推理能力，特别关注数据多样化策略在偏好优化中的作用。

Method: 评估三种数据生成方法：温度采样、思维链提示和蒙特卡洛树搜索，并提出了一种新的结构化方法Diversified-ThinkSolve (DTS)，该方法系统地将问题分解成不同的推理路径。

Result: DTS方法在GSM8K和MATH数据集上分别取得了7.1%和4.2%的提升，计算成本仅为基线的1.03倍，而MCTS方法计算成本是基线的近五倍，提升效果却较低。

Conclusion: 策略性多样化的偏好数据能显著提高大型语言模型的数学推理性能，其中DTS方法在GSM8K和MATH数据集上分别取得了7.1%和4.2%的提升，且计算成本仅略微增加。
...
<details>
  <summary>Details</summary>
Motivation: 确保LLM在角色扮演中输出与角色设定保持一致，研究LLM陈述的信念与其行为一致性的程度。

Method: 建立了一个评估框架，通过增强版的GenAgents角色库和信任博弈，量化LLM信念与其行为的一致性，并研究了多种因素（例如信念类型、信息呈现方式、预测时间范围）的影响，以及如何调整LLM的信念以符合研究目标。

Result: 发现LLM的陈述信念与其角色扮演模拟结果之间存在系统性差异，即使模型似乎编码了合理的信念，也可能无法一致地应用它们，这凸显了识别LLM陈述信念与其模拟行为一致性的方法和时间的必要性。

Conclusion: LLM扮演角色生成合成数据用于人类行为研究，本文发现LLM陈述的信念与其在角色扮演中的实际行为存在系统性差异，即使模型编码了似是而非的信念，也可能无法一致地应用它们。
...
<details>
  <summary>Details</summary>
Motivation: 研究空间囚徒困境博弈中强化学习的静态智能体如何通过不同的机制（包括噪声注入、不同类型的学习算法和邻居的收益知识）学习合作。

Method: 使用独立的多智能体Q学习算法研究空间囚徒困境博弈中稀释和移动性的影响。

Result: 观察到一系列影响，包括具有固定更新规则的游戏在性质上可能等同于具有学习更新规则的游戏，以及在定义多个动作时，种群之间会形成共生互利效应。

Conclusion: 研究表明，具有固定更新规则的游戏在性质上可能等同于具有学习更新规则的游戏，并且在定义多个动作时，种群之间会形成共生互利效应。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM规划能力提升受限于数据生成和评估的瓶颈，因此需要一个自动化系统来生成和评估规划问题。

Method: 提出NL2FLOW系统，用于生成自然语言、结构化中间表示和PDDL形式的规划问题，并评估LLM生成的计划质量。

Result: 生成包含2296个问题的自动化工作流数据集，评估多个开源LLM，顶级模型在生成有效和最优计划方面分别达到86%和69%的成功率。中间步骤的引入可能降低性能。

Conclusion: 该论文介绍了NL2FLOW系统，用于自动生成规划问题并评估LLM的规划能力，实验结果表明，顶级模型在生成有效和最优计划方面取得了显著成果，但中间步骤的引入可能降低性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有信仰修正研究过于关注假设和约束，忽略了对修正机制能力的研究。

Method: 分析现有信仰修正方法的优缺点，并提出新的分析视角。

Result: 提出了一种新的分析信仰修正机制的视角，关注其能力而非约束，并对多种修正机制的能力进行了分析。

Conclusion: 本文批判了信仰修正领域过多关注假设而忽视对现有方法的分析，提出应关注修正机制的能力而非约束，例如可塑性、等同性、教条性等，并分析了不同修正机制（词典式、自然、受限、激进等）所拥有的能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的关键词生成方法存在依赖大规模查询关键词对数据、缺乏在线多目标性能监控和优化、关键词选择质量控制薄弱等局限性。

Method: 提出了一种名为OMS的关键词生成框架，该框架无需训练数据、在线监控性能并据此调整、多目标优化、能够自主评估关键词质量。

Result: 实验表明，OMS优于现有方法。

Conclusion: OMS框架在基准测试和实际广告活动中优于现有方法，消融研究和人工评估证实了每个组件的有效性和生成的关键词质量。
...
<details>
  <summary>Details</summary>
Motivation: 为了实现自主科学研究，并服务于非专业人士，本文提出了一个能够执行复杂科学实验的AI原生自主实验室。

Method: 该系统基于模型、实验和仪器的协同设计理念，自主管理仪器，制定实验流程和优化策略。

Result: 该自主实验室成功地执行了包括合成、转录、扩增和测序在内的基本核酸功能实验，并在疾病诊断、药物研发和信息存储等领域具有应用潜力。

Conclusion: 本文介绍了一个AI驱动的自主实验室，能够执行复杂的生物分子工程实验，并能同时服务多个用户请求，实现了与人类科学家相当的实验性能，并提高了仪器利用率和实验效率。
...
<details>
  <summary>Details</summary>
Motivation: 为了响应AI可解释性原则的需求，并促进AI的更好社会应用，提升机器学习的可理解性和可解释性。

Method: 使用范畴论，特别是伴随函子对，对多元线性回归模型进行建模，阐明残差和参数之间的结构相互作用。

Result: 提出了多元线性回归模型的范畴论公式，即高斯-马尔可夫伴随，明确描述了参数和残差之间信息流动的对偶性，并将其定位为监督学习的扩展指称语义的一个实例。

Conclusion: 本文通过范畴论的视角重新构建机器学习模型，为理解和构建AI系统提供语义框架，以提高机器学习的可理解性和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何提高大型语言模型的推理能力，关注Coq定理证明。

Method: 引入概念级度量评估任务清晰度，使用选择性概念展开丰富任务描述，并采用规划器-执行器架构。

Result: 添加结构化语义上下文后，清晰度评分提高1.85倍（44.5%→82.3%），证明成功率提高2.1倍（21.8%→45.8%），优于现有技术。小型模型微调后，证明成功率达48.6%。

Conclusion: 提升大型语言模型任务清晰度可以增强其推理能力，尤其在Coq定理证明中。结构化语义上下文可以显著提高任务清晰度评分和证明成功率，小型模型微调后效果更佳。
...
<details>
  <summary>Details</summary>
Motivation: 提高AI代理在MLE-bench上的性能，这是一个具有挑战性的基准，代理在Kaggle竞赛中竞争以解决现实世界的机器学习问题。

Method: 将AI研究代理形式化为搜索策略，迭代地使用算子修改候选解决方案。

Result: 最佳搜索策略和算子集组合在MLE-bench lite上取得了最先进的结果，将获得Kaggle奖牌的成功率从39.6%提高到47.7%。

Conclusion: AI研究代理在自动设计、实现和训练机器学习模型方面显示出巨大的潜力，以加速科学进步。研究人员通过设计和系统地改变不同的算子集和搜索策略（贪婪、MCTS、进化），证明了它们的相互作用对于实现高性能至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 研究责任在人工智能领域的计算复杂性。

Method: 计算复杂性分析

Result: 无责任分散和无责任缺口决策机制集合的计算复杂度分别为Π₂完全和Π₃完全，两者的交集为Π₂完全。

Conclusion: 本文探究了集体决策中责任的两个重要属性——责任分散和责任缺口——的计算复杂性，并证明了无责任分散和无责任缺口决策机制集合分别为Π₂完全和Π₃完全，而两者的交集为Π₂完全。
